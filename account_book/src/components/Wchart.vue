...
export default {
  ...
  async mounted() {
    ...
    if (user) {
      await this.fetchCharData(user.user_id);
    }
    google.charts.load("current", { packages: ["corechart"] });
    google.charts.setOnLoadCallback(this.drawChart.bind(this));
  },
  methods: {
    async fetchCharData(userId) {
      try {
        ...
        const minusData = withdraws.filter(function(withdraw) {
          const year = new Date(withdraw.update_date).getFullYear();
          return year === 2024;
        });

        const monthlyData = minusData.reduce(function (acc, withdraw) {
        ...
        this.totalWithdrawals = Object.values(monthlyData).reduce(function (
          acc,
          amount
        ) {
          return acc + amount;
